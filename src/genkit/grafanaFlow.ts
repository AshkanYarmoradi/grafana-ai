...
import { grafanaApiRequest, GrafanaApiError } from '@/genkit/grafanaApi';
...
export const listDatasources = ai.defineTool(
    ...
    async (): Promise<Array<{uid: string, name: string, type: string}>> => {
...
export const queryDatasource = ai.defineTool(
    ...
    async (params: {
        ...
        const { datasourceUid, rawQuery, from, to } = params;
    ...
    async (
        ...
        const generateResponse = await ai.generate({
            ...
            prompt: [
                ...
                `2. Formulate a native query for that datasource (PromQL, LogQL, SQL, etc.).`,
                `3. If the user's question implies a time range (e.g., "in the last 3 hours", "yesterday"), determine the 'from' and 'to' values. Otherwise, you can omit them to use the default.`,
                `4. Respond with ONLY a valid JSON object containing the datasource 'uid', the 'rawQuery', and optional 'from' and 'to' fields.`
            ].join('\n'),
        ...
        const { uid, query, from = 'now-1h', to = 'now' } = generateResponse.output;
